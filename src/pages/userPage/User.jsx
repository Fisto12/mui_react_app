import React from 'react'
import './user.css'
import { CalendarToday, LocationSearching, MailOutlined, PermIdentity, PhoneRounded, Publish } from '@mui/icons-material'
import { Link } from 'react-router-dom'


export default function User() {
  return (
    <div className='user'>
        <div className="usertitleContainer">
        <h1 className="usertitle">Edit User</h1>
        <Link to='/newUser'>
        <button className="userAddbtn">Create</button>
        </Link>
    </div>
    <div className="usercontainer">
        <div className="usershow">
            <div className="usershowtop">
                 <div className="usershowjobtitle">
                    <span className="usershowusername">Anna becker</span>
                    <span className="showuserjob">Software Engineer</span>
                </div>
            </div>
            <div className="usershowbottom">
                <span className="useshowtit">Account details</span>
                <div className="usershowinfo">
                <PermIdentity className='usershowicon'/>
                <span className="showinfotitle">annabeck99</span>
                </div>
                <div className="usershowinfo">
                <CalendarToday className='usershowicon'/>
                <span className="showinfotitle">10.12.1999</span>
                </div>
                <span className="useshowtit">Contact Details </span>
                <div className="usershowinfo">
                <PhoneRounded className='usershowicon'/>
                <span className="showinfotitle">07067796826</span>
                </div>
                <div className="usershowinfo">
                <MailOutlined className='usershowicon'/>
                <span className="showinfotitle">annabeck99@gmail.com</span>
                </div>
                <div className="usershowinfo">
                <LocationSearching className='usershowicon'/>
                <span className="showinfotitle">wakajaye ajagba</span>
                </div>
               
            </div>
        </div>
        <div className="userupdate"> 
        <span className="userupdatetitle">Edit</span>
        <form className="userupdateform">
          <div className="userupdateleft">
              <div className="userupdateitem">
                  <label>Usename</label>
                  <input type="text" placeholder='annabeck' className='userupdinput'/>
              </div>
              <div className="userupdateitem">
                  <label>Full name</label>
                  <input type="text" placeholder='anna becker' className='userupdinput'/>
              </div>
              <div className="userupdateitem">
                  <label>Email</label>
                  <input type="text" placeholder='annabeck99@gmail.com' className='userupdinput'/>
              </div>
              <div className="userupdateitem">
                  <label>Phone</label>
                  <input type="text" placeholder='07067796826' className='userupdinput'/>
              </div>
              <div className="userupdateitem">
                  <label>Phone</label>
                  <input type="text" placeholder='wakajaye ajagba' className='userupdinput'/>
              </div>
          </div>
          <div className="userupdateright">
              <div className="userupdtupoad">
                <img src="data:image/jpeg;base64," alt="" className="userupdteimg" />
               <label htmlFor="file"><Publish className='userupdticon'/> </label>
               <input type="file" id='file' style={{display:'none'}} />
              </div>
              <button className="userupdtbtn">Update</button>
          </div>
        </form>
        </div>
    </div>
    </div>
  )
}
